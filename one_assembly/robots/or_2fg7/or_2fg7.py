...
def prepare_ms():
    ...
    lf_lnk = orbms.Link.from_file(
        ...
        loc_rotmat=oum.rotmat_from_euler(0, 0, -np.pi / 2),
        loc_pos=oum.vec(0.0, -0.0223 + 0.0005, -0.1024).astype(np.float32),
        collision_type=ouc.CollisionType.MESH,
    ...
    jnt_lf = orbms.Joint(
        ...
        pos=np.array([0, -0.0085, 0], dtype=np.float32),
        lmt_lo=0.0085, lmt_up=0.0225)
    jnt_rf = orbms.Joint(
        jnt_type=ouc.JntType.PRISMATIC,
...
class OR2FG7(oreb.EndEffectorBase, oreb.GripperMixin):
    ...
    def __init__(self):
        super().__init__(loc_tcp_tf=oum.tf_from_rotmat_pos(pos=(0, 0, 0.2)))
        self.jaw_range = np.array([0.017, 0.055], dtype=np.float32)  # min, max
    ...
    def clone(self):
        ...
        new.set_jaw_width((self.qs[0] + self.qs[1]))
        return new
