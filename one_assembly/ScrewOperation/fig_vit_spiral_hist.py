...
def main(args, config: ScrewConfig):
    ...
    for _, row in df.iterrows():
        ...
        offline_corr_pos = tcp_pos - (rotmat[:, 0] * dx_pred + rotmat[:, 1] * dy_pred) * sgn
        ...
        rows.append({
            ...
            "x_corr": x_corr,
            "y_corr": y_corr,
            "x_corr_offline": float(offline_corr_pos[0]),
            "y_corr_offline": float(offline_corr_pos[1]),
            "pos_err_m": pos_err
        })
    ...
    plt.scatter(x0[0], y0[0], c="orange", s=100, label="Start TCP position")
    plt.scatter(x0, y0, c="black", s=50, label="Before correction")
    plt.scatter(x1, y1, c="tab:blue", s=30, label="After online correction")
    ...
    r = 1.0 / 1000.0
    circle = plt.Circle((gt_x, gt_y), r, color="red", fill=False, linestyle="--", linewidth=2, label="Success region (1 mm)")
    plt.gca().add_patch(circle)

    square_range = args.num_accu * 20
    plt.xlim(gt_x - square_range, gt_x + square_range)
    plt.ylim(gt_y - square_range, gt_y + square_range)
    if toggle_label:
        plt.xlabel("x [m]")
        plt.ylabel("y [m]")
